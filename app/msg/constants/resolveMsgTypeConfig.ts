...
export function resolveMsgCopy(type: MessageType): ResolvedMsgTypeInfo {
  ...
  const label = resolveTranslatable(cfg.label as Translatable, translate) ?? ''

  const description = resolveTranslatable(cfg.description as Translatable, translate) ?? ''
  ...
  const cost = resolveTranslatable(costTemplate, translate) ?? ''

  const warning = resolveTranslatable(cfg.warning, translate) ?? ''
