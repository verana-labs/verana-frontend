...
describe('buildVoteMessage', () => {
  it('votes with EXEC_TRY so the deciding vote executes the proposal', () => {
    const message = buildVoteMessage(23, 'verana1member', 'yes')
    const value = MsgVote.decode(MsgVote.encode(message.value as MsgVote).finish())
    expect(message.typeUrl).toBe('/cosmos.group.v1.MsgVote')
    expect(value.proposalId).toBe(BigInt(23))
    expect(value.voter).toBe('verana1member')
    expect(value.option).toBe(VoteOption.VOTE_OPTION_YES)
    expect(value.exec).toBe(Exec.EXEC_TRY)
  })
})
