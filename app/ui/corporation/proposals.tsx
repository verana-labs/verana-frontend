...
const FILTER_KEYS: Record<Filter, string> = {
  ...
  ABORTED: 'corporation.proposals.filter.aborted',
  ...
  return (
    <div className="min-w-[8rem]">
      ...
      <p className="mt-1 text-xs text-gray-500 dark:text-gray-400 whitespace-nowrap">
        {translate('corporation.proposals.tally', { yes: tally.yes, total, threshold })}
      </p>
    </div>
  )
}

function hasEnded(votingPeriodEnd: string | null): boolean {
  if (!votingPeriodEnd) return false
  const end = Date.parse(votingPeriodEnd)
    ...
    <div className="space-y-3">
      ...
      <div>
        <p className="text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400 mb-1">
          {translate('corporation.proposals.votes')}
        </p>
        {votes === undefined ? (
      ...
      {votable || withdrawable || executable ? (
        <div className="flex flex-wrap gap-2">
          ...
          {withdrawable ? (
            ...
            >
              {translate('corporation.proposals.withdraw')}
            </button>
          ) : null}
          {executable ? (
            <button
            ...
            >
              {translate('corporation.proposals.execute')}
            </button>
          ) : null}
        </div>
      ) : null}
  ...
  return (
    <div className="border border-neutral-20 dark:border-neutral-70 rounded-lg">
      ...
      >
        ...
        {proposal.executorResult && proposal.executorResult !== 'NOT_RUN' ? (
          <span className="text-xs text-gray-500 dark:text-gray-400">
            {translate('corporation.proposals.executor')}: {proposal.executorResult}
          </span>
        ) : null}
      ...
      <div className="px-4 pb-3">
        <TallyBar tally={proposal.tally} policy={ctx.policy} />
      </div>
      {open ? (
        <div className="px-4 pb-4">
          <ProposalDetail key={ctx.votesVersion} proposal={proposal} ctx={ctx} />
        </div>
      ) : null}
    </div>
  )
  ...
  return (
    <Card id="proposals">
      <div className="flex items-center justify-between gap-3 mb-4">
        <SectionTitle>{translate('corporation.tab.proposals')}</SectionTitle>
        {ctx.isMember && !composing ? (
          <button
            ...
            className="px-3 py-1.5 bg-primary-600 hover:bg-primary-700 text-white rounded-lg text-sm font-medium"
          >
            {translate('corporation.composer.new')}
          </button>
        ) : null}
      </div>
      ...
      <div className="flex flex-wrap gap-1.5 mb-4">
        {FILTERS.map((entry) => {
          ...
          return (
            ...
            >
              {translate(FILTER_KEYS[entry])} · {count}
            </button>
          )
        })}
      </div>
