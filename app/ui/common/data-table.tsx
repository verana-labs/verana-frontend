                ...
                <th key={String(col.accessor)} className="px-6 py-2">
                  {col.filterType === 'checkbox' ? (
                    <label className="inline-flex items-center">
                      ...
                      />
                      <span className="ml-2 text-sm font-medium text-gray-700">{col.filterLabel ?? col.header}</span>
                    </label>
                  ) : (
                    <input
                      type="text"
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {currentData.map((row, idx) => (
              <tr
                ...
                className={`odd:bg-white even:bg-gray-50 ${onRowClick ? 'cursor-pointer hover:bg-gray-100' : ''}`}
              >
                {columns.map(col => (
          ...
          {pageButtons.map(pageIndex => (
            <button
              ...
              className={`px-3 py-1 rounded-md text-sm ${
                pageIndex === currentPage ? 'bg-blue-500 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
              }`}
            >{pageIndex + 1}</button>
          ))}
          {showEllipsis && (
