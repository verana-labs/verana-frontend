            ...
            {entities && (
              <div className="flex items-center justify-between">
                <h3 className="data-table-title">
                  {resolveTranslatable({ key: 'datatable.your' }, translate)} {entities}
                </h3>
              </div>
            )}
            ...
            <table className="w-full">
              ...
              <tbody>
                ...
                {currentData.map((row, rowIdx) => (
                  ...
                  >
                    {columns.map((col) => {
                      const formatted = col.format ? col.format(row[col.accessor]) : row[col.accessor]
                      ...
                      return (
                        ...
                        >
                          {col.isHtml ? <p dangerouslySetInnerHTML={{ __html: valueStr }} /> : valueStr}
                        </td>
                      )
                    })}
                  </tr>
              ...
              >
                <div className="flex justify-between">
                  <div className="flex flex-col space-y-2">
                    {columns
                      .filter((col) => col.priority === undefined && !col.viewMobileRight)
                      .map((col) => (
                  ...
                  <div className="text-right">
                    {columns
                      .filter((col) => col.viewMobileRight)
                      .map((col) => {
                        const formatted = col.format ? col.format(row[col.accessor]) : row[col.accessor]
                        const valueStr = String(formatted ?? '')
                        return (
                          <div key={String(col.accessor)}>
                            {col.isHtml ? (
                              ...
                              <label
                                className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${col.getClassName ? col.getClassName(row[col.accessor]) : ''}`}
                              >
                                {valueStr}
                              </label>
                            )}
