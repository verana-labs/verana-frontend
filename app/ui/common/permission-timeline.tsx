...
export default function PermissionTimeline({ permissionHistory }: { permissionHistory: PermissionHistory }) {
  const { label, icon, iconBgClass, iconColorClass } = getTimelineStyle(permissionHistory.msg)
  const account = permissionHistory.account ? shortenMiddle(permissionHistory.account, 20) : ''
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-neutral-70 dark:text-neutral-70 mt-1 break-all">
          ...
          {account ? (
            <>
              {' '}
              {tr('permissioncard.timeline.by', 'by')} <span className="font-mono">{account}</span>
            </>
          ) : null}
        </p>
