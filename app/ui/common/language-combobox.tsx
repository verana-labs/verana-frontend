...
import { LanguageOption, languageOptions } from '@/ui/dataview/datasections/gfd'
...
export function LanguageCombobox({ value, onChange, disabled = false, className = '' }: LanguageComboboxProps) {
  ...
  const filtered =
    query === ''
      ? languageOptions
      : languageOptions.filter((opt) => opt.label.toLowerCase().includes(query.toLowerCase()))
  ...
  return (
    ...
    >
      <div className="relative">
        ...
        >
          {filtered.length === 0 ? (
            <div className="px-4 py-2 text-sm text-gray-500 dark:text-gray-400">No languages found.</div>
          ) : (
            filtered.map((opt, i) => (
              <span key={opt.value}>
