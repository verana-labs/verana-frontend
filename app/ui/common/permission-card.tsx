...
const accountActions: PermissionAction[] = [
  ...
  {
    icon: faEye,
    label: t('permissioncard.action.visualizer', 'visualizer'),
    value: 'visualizer',
...
const v4MetaItems: V4Item[] = [
  ...
  {
    label: t('permissioncard.meta.deposit', 'Deposit'),
    attr: 'deposit',
    mono: true,
    format: (v) => formatVNAFromUVNA(String(v)),
  },
  {
    label: t('permissioncard.meta.effectivefrom', 'Effective From'),
    attr: 'effective_from',
    format: (v) => formatDateTime(v as string),
  },
  {
    label: t('permissioncard.meta.effectiveuntil', 'Effective Until'),
    attr: 'effective_until',
    format: (v) => formatDateTime(v as string),
  },
  {
    label: t('permissioncard.meta.country', 'Country'),
    attr: 'country',
    format: (v) => countryNameFromCode(v as string),
  },
...
const v4LifecycleItems: V4Item[] = [
  {
    label: t('permissioncard.lifecycle.created', 'Created'),
    attr: 'created',
    format: (v) => formatDateTime(v as string),
  },
  {
    label: t('permissioncard.lifecycle.createdby', 'Created By'),
    attr: 'created_by',
    mono: true,
    extraActions: accountActions,
  },
  {
    label: t('permissioncard.lifecycle.modified', 'Modified'),
    attr: 'modified',
    format: (v) => formatDateTime(v as string),
  },
  {
    label: t('permissioncard.lifecycle.modifiedby', 'Modified By'),
    attr: 'modified_by',
    mono: true,
    extraActions: accountActions,
  },
  {
    label: t('permissioncard.lifecycle.extended', 'Extended'),
    attr: 'extended',
    format: (v) => formatDateTime(v as string),
  },
  {
    label: t('permissioncard.lifecycle.extendedby', 'Extended By'),
    attr: 'extended_by',
    mono: true,
    extraActions: accountActions,
  },
...
const v4VpItems: V4Item[] = [
  {
    label: t('permissioncard.validationprocess.vpexp', 'VP Expiration'),
    attr: 'vp_exp',
    format: (v) => formatDateTime(v as string),
  },
  {
    label: t('permissioncard.validationprocess.vplaststatechange', 'VP Last State Change'),
    attr: 'vp_last_state_change',
    format: (v) => formatDateTime(v as string),
  },
  {
    label: t('permissioncard.validationprocess.vpvalidatordeposit', 'VP Validator Deposit'),
    attr: 'vp_validator_deposit',
    mono: true,
    format: (v) => formatVNAFromUVNA(String(v)),
  },
  {
    label: t('permissioncard.validationprocess.vpcurrentfees', 'VP Current Fees'),
    attr: 'vp_current_fees',
    mono: true,
    format: (v) => formatVNAFromUVNA(String(v)),
  },
  {
    label: t('permissioncard.validationprocess.vpcurrentdeposit', 'VP Current Deposit'),
    attr: 'vp_current_deposit',
    mono: true,
    format: (v) => formatVNAFromUVNA(String(v)),
  },
  {
    label: t('permissioncard.validationprocess.vpsummarydigestsri', 'VP Summary Digest'),
...
export default function PermissionCard({ selectedNode, path, csTitle, onRefresh }: PermissionCardProps) {
  ...
  const granteeActions = selectedNode.isGrantee ? (selectedNode.permission?.grantee_available_actions ?? []) : []
  const validatorActions = selectedNode.isValidator ? (selectedNode.permission?.validator_available_actions ?? []) : []
  ...
  const lifecycleActionsWithoutSlashing = permissionActionLifecycle.filter(
    (a) => !a.name || !slashingActionNames.has(a.name)
  )
  ...
  const { labelVpState, classVpState } = vpStateColor(
    selectedNode.permission?.vp_state as VpState,
    selectedNode.permission?.vp_exp as string,
    selectedNode.permission?.expire_soon ?? false
  )
  const { labelPermState, classPermState } = permStateBadgeClass(
    selectedNode.permission?.perm_state as PermState,
    selectedNode.permission?.expire_soon as boolean,
    'header'
  )
  ...
  useEffect(() => {
    ...
    logger.info('PermissionCard', {
      txHeight: refreshState.txHeight,
      latestProcessedHeight,
      'ss.mmm': new Date().toISOString().slice(17, 23),
    })
      ...
      {selectedNode.permission && (
        <>
          <div className="pb-6">
            <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-4">
              <div className="flex-1">
                <div className="flex items-center space-x-2 mb-4">
                  <h2 className="text-xl font-bold text-gray-900 dark:text-white break-words">{headerTitle}</h2>
            ...
            {detailBreadcrumb ? (
              <p className="text-sm text-neutral-70 dark:text-neutral-70 mt-2">{detailBreadcrumb}</p>
            ) : null}
                    ...
                    <div className="flex flex-wrap items-center gap-3 text-sm">
                      ...
                      {enrichment.serviceTermsUrl ? (
                        ...
                        >
                          ...
                          <span>
                            {resolveTranslatable({ key: 'permissioncard.grantedservice.terms' }, translate) ??
                              'Terms & Conditions'}
                          </span>
                        </a>
                      ) : null}
                      {enrichment.servicePrivacyUrl ? (
                        ...
                        >
                          ...
                          <span>
                            {resolveTranslatable({ key: 'permissioncard.grantedservice.privacy' }, translate) ??
                              'Privacy Policy'}
                          </span>
                        </a>
                      ) : null}
          ...
          <div className="space-y-8">
            <div className="border-t border-neutral-20 dark:border-neutral-70 pt-6">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">
                {resolveTranslatable({ key: 'permissioncard.meta.title' }, translate)}
              </h3>
            ...
            <div className="border-t border-neutral-20 dark:border-neutral-70 pt-6">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">
                {resolveTranslatable({ key: 'permissioncard.lifecycle.title' }, translate)}
              </h3>
                ...
                {lifecycleActionsWithoutSlashing
                  ...
                  .map((action, idx) => (
                    <ActionFieldButtonModal
                      isActive={activeActionId === String(action.name)}
                      data={selectedNode.permission ?? {}}
                      field={action as ActionFieldProps}
                      key={`${action.name}-${idx}`}
                      onRefresh={(id?: string, txHeight?: number) =>
                        setRefreshState({ joinNode: undefined, id, txHeight })
                      }
                      onClickButton={() =>
                        setActiveActionId(activeActionId === String(action.name) ? null : String(action.name))
                      }
              ...
              <div className="border-t border-neutral-20 dark:border-neutral-70 pt-6">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-lg font-semibold text-gray-900 dark:text-white">
                    {resolveTranslatable({ key: 'permissioncard.validationprocess.title' }, translate)}
                  </h3>
                  <span
                    className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${classVpState}`}
                  >
                    {labelVpState}
                  </span>
                </div>
            ...
            <div className="border-t border-neutral-20 dark:border-neutral-70 pt-6">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">
                {resolveTranslatable({ key: 'permissioncard.businessmodels.title' }, translate)}
              </h3>
            ...
            <div className="border-t border-neutral-20 dark:border-neutral-70 pt-6">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">
                {resolveTranslatable({ key: 'permissioncard.timeline.title' }, translate)}
              </h3>
                ...
                <div className="space-y-4">
                  {permissionHistoryList.map((history, idx) => (
                    <PermissionTimeline
                      ...
                      key={`${history.entity_id}-${history.block_height}-${idx}`}
                    />
                  ))}
                </div>
              ) : (
