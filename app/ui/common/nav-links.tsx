...
export default function NavLinks() {
  ...
  const { acting, memberships, loading, needsSelection, requestSelection } = useCorporationContext()
  ...
  return (
    <nav className="mt-5 flex-1 px-2 space-y-1">
      {links.map((link, idx: number) => {
        ...
        const gated = link.requiresCorporation === true && !acting
        if (gated && !loading && memberships.length === 0) return null
        const opensChooser = gated && needsSelection
        return (
