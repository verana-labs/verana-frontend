      ...
      <div className="space-y-4">
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">
            {resolveTranslatable({ key: 'gettoken.title' }, translate)}
          </label>
            ...
            {getVNALinks.map((item, idx) => {
              ...
              return (
                <div key={rowId} className="topup-service-item">
                  <label
                    className="relative flex items-center p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg border border-neutral-20 dark:border-neutral-70 hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer transition-colors"
                  ...
                  >
                    <div className="w-4 h-4 border-2 border-gray-300 dark:border-gray-500 rounded-full mr-3 flex-shrink-0">
                      <div
                        className={`w-full h-full rounded-full bg-primary-600 transition-transform radio-dot ${isActive ? 'scale-100' : 'scale-0'}`}
                      ></div>
                    </div>
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center space-x-2">
                          {item.iconsPay.map((iconPay, idx) => (
                            <FontAwesomeIcon key={idx} icon={iconPay.icon} className={iconPay.iconClass} />
                          ))}
                        </div>
                      </div>
                    </div>
                  ...
                  {isActive && (
                    <div
                      className="ml-7 mt-3 p-4 bg-white dark:bg-gray-800 rounded-lg border border-neutral-20 dark:border-neutral-70"
                      data-service="coinbase"
                    >
                      <div className="flex flex-col md:flex-row gap-6">
                        ...
                        <div className="flex-1">
                          <h4 className="text-sm font-semibold text-gray-900 dark:text-white mb-2">
                            {resolveTranslatable({ key: 'gettoken.instructions' }, translate)}
                          </h4>
