    ...
    <footer className="border-t border-neutral-20 dark:border-neutral-70 pt-8">
      <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
        <div className="col-span-1 md:col-span-2">
          ...
          <p className="text-gray-600 dark:text-gray-400 mb-4 max-w-md">{configFooter.msg}</p>
                    ...
                    <li key={`${i}-${j}`}>
                      <Link href={link.href} target="_blank">
                        <span
                          className={
                            link.className ??
                            'text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white text-sm cursor-pointer'
                          }
                        >
                          {link.label}
                        </span>
      ...
      <div className="mt-8 pt-4 border-t border-neutral-20 dark:border-neutral-70 flex flex-col sm:flex-row justify-between items-center">
        <p className="text-sm text-neutral-70 dark:text-neutral-70">
          ...
          {configFooter.copyright}
        </p>
        <div className="flex space-x-6 mt-4 sm:mt-0">
          {veranaLinks.map((link, idx) => {
            return (
              <Link key={`veranaLink-${idx}`} href={link.href} target="_blank">
                <span className="text-sm text-neutral-70 dark:text-neutral-70 hover:text-gray-900 dark:hover:text-white cursor-pointer">
                  {link.label}
                </span>
              </Link>
            )
