...
type PermissionTreeProps = {
  ...
  setNodeRequestParams?: (nodeId: string | undefined, type: string | undefined, validatorId: string | undefined) => void
  refreshRoot?: () => void
  onConnect?: () => void
  onRetryFetch?: () => void
}
...
export default function PermissionTree({
  ...
  csDescription,
  csStatus,
  csIssuerPermManagementMode,
  csVerifierPermManagementMode,
  trTitle,
  csId,
  trId,
  isTrController,
  ...
  return (
    <>
      ...
      {type === 'participants' && trTitle && trId ? <TrustRegistryBreadcrumb trId={trId} trDid={trTitle} /> : null}
            ...
            <div className="flex flex-wrap items-center gap-x-4 gap-y-2">
              <label className="flex items-center space-x-2 cursor-pointer">
                ...
                <span className="text-sm text-gray-700 dark:text-gray-300 whitespace-nowrap">
                  {resolveTranslatable({ key: 'participants.show.weight' }, translate)}
                </span>
              </label>
              <label className="flex items-center space-x-2 cursor-pointer">
                <input
                ...
                />
                <span className="text-sm text-gray-700 dark:text-gray-300 whitespace-nowrap">
                  {resolveTranslatable({ key: 'participants.show.businessrules' }, translate)}
                </span>
              </label>
              <label className="flex items-center space-x-2 cursor-pointer">
                <input
                ...
                />
                <span className="text-sm text-gray-700 dark:text-gray-300 whitespace-nowrap">
                  {resolveTranslatable({ key: 'participants.show.stats' }, translate)}
                </span>
              </label>
            </div>
          ) : null}
          ...
          {type === 'participants' && isTrController ? (
            ...
            >
              ...
              <span className="text-sm font-medium">
                {resolveTranslatable({ key: 'participants.action.newperm' }, translate)}
              </span>
            </button>
          ) : null}
        </div>
      ...
      {selectedNode ? (
        <div ref={permissionCardRef}>
          <PermissionCard
            ...
            path={path}
            csTitle={csTitle ?? ''}
            onRefresh={(perm: Permission) => refreshNode(perm)}
          />
        </div>
      ) : null}
    </>
