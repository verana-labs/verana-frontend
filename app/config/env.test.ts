...
process.env.NEXT_PUBLIC_VERANA_REST_ENDPOINT_METRICS = 'https://legacy.example/verana/metrics/v1'
process.env.NEXT_PUBLIC_VERANA_WEBSOCKET = 'wss://legacy.example/verana/indexer/v1/events'

import { VERANA_REST_ENDPOINT_METRICS, VERANA_REST_ENDPOINT_TRUST_DEPOSIT, VERANA_WEBSOCKET } from '@/config/env'

describe('indexer endpoints', () => {
  it('derives every route from the indexer base url and ignores legacy per-module variables', () => {
    expect(VERANA_REST_ENDPOINT_METRICS).toBe('https://idx.example/v4/metrics')
