  ...
  const steps: Step[] = [
    {
      ...
      description:
        ...
        'Review and accept the Ecosystem Governance Framework (EGF) before continuing',
    },
    {
      id: 2,
      title: resolveTranslatable({ key: 'join.createpermission.title' }, translate) ?? 'Confirm & Submit',
      description:
        nodeJoin.validationProcessAction === 'MsgCreatePermission'
          ? nodeJoin.type === 'VERIFIER'
            ? (resolveTranslatable({ key: 'join.createpermission.description.open.verifier' }, translate) ??
              'As the schema is OPEN to new verifiers, you just need to self-create your VERIFIER permission by filling this form.')
            : (resolveTranslatable({ key: 'join.createpermission.description.open.issuer' }, translate) ??
              'As the schema is OPEN to new issuers, you just need to self-create your ISSUER permission by filling this form.')
          : (resolveTranslatable({ key: 'join.createpermission.description.vp' }, translate) ??
            'You must run a Validation Process (VP) to obtain a permission for this schema. Start the Validation Process by filling this form.'),
    },
  ]
