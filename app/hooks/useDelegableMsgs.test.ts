...
import { proposalMetadata, type TxConfirmRequest, type TxConfirmResult } from '@/lib/tx-preview'
...
function deps(
  ...
  confirm: (request: TxConfirmRequest) => TxConfirmResult | null = echo
) {
  ...
  const value: DelegableMsgsDeps = {
    ...
    actingCorporation,
    loading: false,
    actingCorporationNow: () => actingCorporation,
    notify,
    confirmTx,
    ...overrides,
  }
...
describe('confirmDelegableMsgs', () => {
  ...
  it('refuses when no corporation is acting', async () => {
    const { deps: d, notify, confirmTx } = deps({ actingCorporation: null })
    expect(await confirmDelegableMsgs(d, args)).toBeNull()
    expect(notify).toHaveBeenCalledWith('Create or select a corporation before continuing.', 'error')
    expect(confirmTx).not.toHaveBeenCalled()
  })
  ...
  it('refuses when the account is neither operator nor member', async () => {
    const {
      ...
      notify,
      confirmTx,
    } = deps({ actingCorporation: membership({ grantedMessageTypes: [], member: false }) })
    expect(await confirmDelegableMsgs(d, args)).toBeNull()
  ...
  it('carries the severity and the existing warning copy of a revocation', async () => {
    const { deps: d, confirmTx } = deps({ actingCorporation: membership({ grantedMessageTypes: [REVOKE] }) })
  ...
  it('falls back to a proposal and offers the default title to the composer', async () => {
  ...
  it('refuses to broadcast when the acting corporation changed while confirming', async () => {
    const { deps: d, notify } = deps({
      actingCorporationNow: () => membership({ corporation: { id: 99, policyAddress: 'x', did: 'y' } }),
    })
    expect(await confirmDelegableMsgs(d, args)).toBeNull()
