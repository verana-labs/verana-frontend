...
function grant(overrides: Partial<FeeGrant> = {}): FeeGrant {
  return { msgTypes: [CREATE], spendLimit: null, remainingSpend: null, ...overrides }
}
...
describe('parseFeeGrants', () => {
  ...
  it('drops malformed entries and fields instead of throwing', () => {
    ...
    ).toEqual([grant({ spendLimit: [] }), grant({ msgTypes: [] })])
  })
})
...
describe('feeGrantCovering', () => {
  it('covers every message type when the grant lists none', () => {
    const grant = { msgTypes: [], spendLimit: null, remainingSpend: null }
  ...
  it('refuses a grant whose spend limit is present but unreadable', () => {
    const [unreadable] = parseFeeGrants({
      fee_grants: [{ msg_types: [CREATE], spend_limit: [{ denom: 'uvna', amount: 5 }], remaining_spend: [] }],
    })
    expect(feeGrantCovering([unreadable], CREATE, '1')).toBeNull()
    const [notAList] = parseFeeGrants({ fee_grants: [{ msg_types: [CREATE], spend_limit: 'nope' }] })
    expect(feeGrantCovering([notAList], CREATE, '1')).toBeNull()
  })
  ...
  it('trusts the active filter for a periodic grant whose cycle boundary has passed', () => {
    const [periodic] = parseFeeGrants({
      fee_grants: [{ msg_types: [CREATE], expiration: '2020-01-01T00:00:00Z', period: '86400s' }],
    })
    expect(feeGrantCovering([periodic], CREATE, '1')).toEqual(grant())
  })
})
