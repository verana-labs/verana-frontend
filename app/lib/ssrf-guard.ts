...
function embeddedIpv4(bytes: Uint8Array, offset = 12): number {
...
function isPrivateIpv6(bytes: Uint8Array): boolean {
  ...
  if (hasPrefix(bytes, [0x00, 0x64, 0xff, 0x9b])) return isPrivateIpv4(embeddedIpv4(bytes))
  if (hasPrefix(bytes, [0x20, 0x02])) return isPrivateIpv4(embeddedIpv4(bytes, 2))
  if (hasPrefix(bytes, [0x20, 0x01, 0x00, 0x00]) || hasPrefix(bytes, [0x20, 0x01, 0x0d, 0xb8])) return true
