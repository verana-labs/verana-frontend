...
function asSpendLimit(value: unknown): DenomAmount[] | null {
  if (value === undefined || value === null || (Array.isArray(value) && value.length === 0)) return null
  return asDenomAmounts(value) ?? []
}
  ...
  for (const entry of envelope.fee_grants) {
    ...
    grants.push({
      msgTypes: Array.isArray(grant.msg_types)
        ...
        : [],
      spendLimit: asSpendLimit(grant.spend_limit),
...
export function feeGrantCovering(grants: FeeGrant[], msgType: string, feeAmountUvna: string): FeeGrant | null {
  const fee = asAmount(feeAmountUvna)
  return grants.find((grant) => coversMsgType(grant, msgType) && coversFee(grant, fee)) ?? null
}
